...
class MinePageVM {
  ...
  Future<void> initData([String bucketId = ""]) async {
    ...
    UploadService().addListener(_onUploadServiceUpdate);
    await initDatas(bucketId);

    checkFilesSyncState();
    _checkUploadingState();
  }

  void _onUploadServiceUpdate() {
    var tasks = UploadService().tasks;
    bool changed = false;

    for (var task in tasks) {
      try {
        var file = mineEntries.localEntries.firstWhere((e) => e.uri == task.filePath);
        
        bool newUploading = (task.status == UploadTask.STATUS_PENDING || task.status == UploadTask.STATUS_UPLOADING);
        bool newError = (task.status == UploadTask.STATUS_FAILED);

        if (file.isUploading != newUploading || file.isError != newError) {
           file.isUploading = newUploading;
           file.isError = newError;
        ...
        // File not in current list
      }
    }
    
    if (changed) {
      mineEntries.notifyListeners();
    }
  }

  void _checkUploadingState() {
    var tasks = UploadService().tasks;
    for (var task in tasks) {
      if (task.status == UploadTask.STATUS_PENDING || task.status == UploadTask.STATUS_UPLOADING) {
         try {
           var file = mineEntries.localEntries.firstWhere((e) => e.uri == task.filePath);
           file.isUploading = true;
           file.isError = false;
         } catch (e) {
           // File might not be in the current view list
         }
      } else if (task.status == UploadTask.STATUS_FAILED) {
          try {
           var file = mineEntries.localEntries.firstWhere((e) => e.uri == task.filePath);
  ...
  Future<void> syncFiles() async {
    for (var index = 0; index < mineEntries.syncMarks.length; index++) {
      if (mineEntries.syncMarks[index]) {
        var file = mineEntries.localEntries[index];
        file.isUploading = true;
        file.isError = false;
        
        // Ensure MD5 is present
        if (file.md5 == null || file.md5!.isEmpty) {
           // Should have been generated by checkFilesSyncState
           file.md5 = await getFileHash(file.uri);
        }
        
        // Add to upload service
        File fileInfo = File(file.uri);
        int length = 0;
        if(fileInfo.existsSync()){
          length = await fileInfo.length();
        }

        await UploadService().addTask(
            file.uri, 
            file.name, 
            file.md5!, 
            file.bucket,
    ...
    // Clear selection
    mineEntries.syncMarks = List.filled(mineEntries.localEntries.length, false);
    mineEntries.notifyListeners();
  }
}
